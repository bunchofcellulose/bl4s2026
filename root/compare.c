void compare()
{
    ...
    const int nFiles = 5; // Must match the number of files above
    const char *treeName = "Calorimeter";
    const char *varName = "fEdep";
    ...
    // Create a Legend
    TLegend *leg = new TLegend(0.65, 0.7, 0.4, 0.9);
    leg->SetTextSize(0.03);

    // Create a Stack (helps manage the axes automatically)
    THStack *hs = new THStack("hs", "Beam Position (Negative Beam); X [cm]; Normalized Counts");
